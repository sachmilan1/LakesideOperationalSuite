...
export default function Vin() {
  ...
  const canSearch = vin.trim().length > 0;

  const onSearch = () => {
    if (!canSearch) return;
    router.replace({
      pathname: "/shop/workOrders/search/vin/[vin]",
      ...
      <BlurView intensity={80} tint="light" style={{ flex: 1 }}>
        <SafeAreaView style={{ flex: 1 }}>
          <View style={styles.container}>
            <Text style={styles.title}>Search by VIN</Text>
...
const styles = StyleSheet.create({
  ...
  back: { backgroundColor: "#ef4444" }, // lighter red for secondary action
